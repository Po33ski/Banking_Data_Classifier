...
def label_id_to_name(label_id: int) -> str:
    """
    Translate a numeric label id (as stored in the dataset) to its human-readable
    class name defined in CLASS_ID_TO_NAME.
    """
    try:
        return CLASS_ID_TO_NAME[int(label_id)]
    except (IndexError, ValueError, TypeError) as exc:
        raise ValueError(f"Unknown label id: {label_id}") from exc


def build_label_lookup(df: pd.DataFrame) -> Dict[int, str]:
    """
    Build a mapping between numeric label ids and their human-readable names
    based on the dataframe columns.
    """
    required_columns = {"label", "real_label"}
    missing = required_columns.difference(df.columns)
    if missing:
        missing_cols = ", ".join(sorted(missing))
        raise KeyError(f"Missing columns for label lookup: {missing_cols}")

    lookup_source = (
        df.loc[:, ["label", "real_label"]]
        .dropna(subset=["label", "real_label"])
        .drop_duplicates(subset="label")
    )
    return {int(label): str(real_label) for label, real_label in lookup_source.values}
